...
function M.setup(opts)
  local ok_signs, signs = pcall(require, "dap.ui.signs")
  if ok_signs and opts.signs then
    pcall(signs.setup)
  end

  local ok_hl, highlights = pcall(require, "dap.ui.highlights")
  if ok_hl and opts.highlights then
    pcall(highlights.setup)
  end

  if opts.enable then
    local ok_provider, provider = pcall(require, "dap.ui.provider")
    if ok_provider then
  ...
  if opts.virtual_text then
    local ok_vt, virtual_text = pcall(require, "dap.ui.virtual_text")
    if ok_vt then

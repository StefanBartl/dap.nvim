---@module 'dap.health'
---@brief :checkhealth dap — environment, lib.nvim deps, adapters, externals.
...
function M.check()
  ...
  if vim.g.loaded_dap then
    vim.health.ok("plugin loaded (vim.g.loaded_dap = " .. tostring(vim.g.loaded_dap) .. ")")
  else
  ...
  local ui_opts = require("dap.config").get().ui or {}
  ...
  local active = require("dap.ui.provider").active()
  if active then
  ...
  if require("dap.bindings.which_key").available() then

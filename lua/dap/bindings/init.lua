...
function M.setup(cfg)
  require("dap.bindings.usercmds").setup()
  ...
  if cfg.keymaps.enable then
    ...
    local ok, err = pcall(require("dap.bindings.keymaps").setup, cfg.keymaps)
    if not ok then
      notify.warn("Skipped keymaps: " .. tostring(err))
    end

    if cfg.which_key.enable then
      pcall(require("dap.bindings.which_key").setup, cfg.keymaps.prefix)
    end
  end

  require("dap.bindings.autocmds").setup(cfg.autocmds)
end

...
function M.setup(opts)
  if M._initialized then
    require("dap.utils.notify").warn("Already initialized")
    return false
  end
  ...
  if not ok_core then
    require("dap.utils.notify").error("Failed to load core module")
    return false
  end

  local ok_init, err = pcall(core.setup, cfg)
  if not ok_init then
    require("dap.utils.notify").error(string.format("Core initialization failed: %s", err))
    return false
  end
  ...
  if ok_adapters and type(adapters_mod.register_all) == "function" then
    ...
    if not adapter_ok then
      require("dap.utils.notify").warn(string.format("Adapter registration failed: %s", adapter_err))
    end
  end
  ...
  if ok_configs and type(configurations_mod.load_all) == "function" then
    ...
    if not config_ok then
      require("dap.utils.notify").warn(string.format("Configuration loading failed: %s", config_err))
    end
  end
  ...
  require("dap.bindings").setup(cfg)

...
function M.setup(cfg)
  require("dap_nvim.bindings.usercmds").setup()
  ...
  if cfg.keymaps.enable then
    ...
    local ok, err = pcall(require("dap_nvim.bindings.keymaps").setup, cfg.keymaps)
    if not ok then
      notify.warn("Skipped keymaps: " .. tostring(err))
    end

    if cfg.which_key.enable then
      pcall(require("dap_nvim.bindings.which_key").setup, cfg.keymaps.prefix)
    end
  end

  require("dap_nvim.bindings.autocmds").setup(cfg.autocmds)
end
